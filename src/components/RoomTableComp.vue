  ...
  <div class="item">
    <div class="details">
      <table>
        ...
        <th>Humidity</th>
        <th>Temperature</th>
        <tr v-for="room in meetingRoomArr" :key="room.name"><td>{{room.name}}</td><td>{{room.hum}}</td><td>{{room.temp}}°F</td></tr>
      </table>
     <slot></slot>
    </div>
  </div>
...
export default class MapComponent extends Vue {
  ...
  mounted() {
    ...
    let roomArr = [{name: "The Mitten", hum: 20, temp: 75},{name: "Great Lakes", hum: 20, temp: 73}, {name: "Theater", hum: 20, temp: 76}, {name: "Better Made", hum: 20, temp: 75},{name: "Big Red", hum: 20, temp: 71}, {name: "Break Room", hum: 20, temp: 70}]
